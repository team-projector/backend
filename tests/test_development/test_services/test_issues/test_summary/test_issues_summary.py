...
def test_problems(user):
    ...
    IssueFactory.create_batch(
        1, user=user, total_time_spent=0, due_date=datetime.now()
    )
...
def test_team_summary(db):
    ...
    TeamMemberFactory.create(
        user=user_1, team=team_1, roles=TeamMember.roles.DEVELOPER,
    )
    IssueFactory.create_batch(
        5,
        user=user_1,
    ...
    _check_team_stats(
        summary, team_1, issues_opened_count=5, percentage=5 / 8, remains=500
    )
    _check_team_stats(
        summary, team_2, issues_opened_count=3, percentage=3 / 8, remains=900
    )
...
def test_time_spents_by_milestone(user):
    ...
    IssueSpentTimeFactory.create(
        date=datetime.now(), user=user, base=issue_1, time_spent=100,
    )
    ...
    IssueSpentTimeFactory.create(
        date=datetime.now(), user=user, base=issue_2, time_spent=300,
    )
...
def test_resolve_issues_summary(user, client):
    ...
    client.user = user
    info = AttrDict({"context": client, "field_asts": [{}], "fragments": {}})

    summary = resolve_issues_summary(parent=None, info=info, user=user.id)
...
def test_resolve_issues_project_summaries(user):
    ...
    parent = AttrDict({"queryset": Issue.objects.all()})
...
def _check_project_stats(
    ...
    stats = next(
        (item for item in data.projects if item.project == project), None
    )
...
def _check_team_stats(
    ...
    stats = next((item for item in data.teams if item.team == team), None)
