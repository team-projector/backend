...
class SalariesTests(BaseAPITest):
    ...
    def test_list(self):
        ...
        spend_1 = IssueSpentTimeFactory(user=self.user, salary=salary_1,
                                        time_spent=60)
        spend_2 = IssueSpentTimeFactory(user=self.user, salary=salary_1,
                                        time_spent=30)
        spend_3 = IssueSpentTimeFactory(user=self.user, salary=salary_2,
                                        time_spent=90)
        ...
        self.assertTrue(response.data['results'][0]['time_spent'],
                        spend_3.time_spent)
        self.assertTrue(response.data['results'][0]['issue']['id'],
                        spend_3.base.id)
    ...
    def test_permissions(self):
        ...
        spend_developer = IssueSpentTimeFactory(user=self.user,
                                                salary=salary_developer,
                                                time_spent=60)

        developer_another_team = UserFactory.create()
        TeamMemberFactory.create(team=TeamFactory.create(),
                                 user=developer_another_team)
        salary_developer_another = SalaryFactory.create(
            user=developer_another_team)
        IssueSpentTimeFactory(user=self.user, salary=salary_developer_another,
                              time_spent=30)

        team_leader = UserFactory.create()
        TeamMemberFactory.create(team=self.team, user=team_leader,
                                 roles=TeamMember.roles.leader)
        salary_team_leader = SalaryFactory.create(user=team_leader)
        spend_team_leader = IssueSpentTimeFactory(user=self.user,
                                                  salary=salary_team_leader,
                                                  time_spent=90)

        self.set_credentials(developer)
        response = self.client.get(
            f'/api/salaries/{salary_developer.id}/time-expenses')

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['results'][0]['id'], spend_developer.id)

        response = self.client.get(
            f'/api/salaries/{salary_team_leader.id}/time-expenses')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(response.data['detail'],
                         'You can\'t view user salaries')

        response = self.client.get(
            f'/api/salaries/{salary_developer_another.id}/time-expenses')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
