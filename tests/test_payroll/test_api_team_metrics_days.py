...
class ApiTeamMetricsDaysTests(CheckUserProgressMetricsMixin,
    ...
    def test_simple(self):
        self._create_spent_time(timezone.now() - timedelta(days=4),
                                timedelta(hours=3))
        self._create_spent_time(timezone.now() - timedelta(days=2, hours=5),
                                timedelta(hours=2))
        self._create_spent_time(timezone.now() - timedelta(days=1),
                                timedelta(hours=4))
        self._create_spent_time(timezone.now() - timedelta(days=1, hours=5),
                                -timedelta(hours=3))
        ...
        self.assertEqual(len(developer_metrics['metrics']),
                         (end - start).days + 1)
    ...
    def test_loading_day_already_has_spends(self):
        ...
        self._create_spent_time(timezone.now(), timedelta(hours=1),
                                issue=issue_2)
        self._create_spent_time(timezone.now(), timedelta(hours=2),
                                issue=issue_2)
    ...
    def test_not_in_range(self):
        ...
        self._create_spent_time(timezone.now() - timedelta(days=5, hours=5),
                                timedelta(hours=2))
        self._create_spent_time(timezone.now() - timedelta(days=1),
                                timedelta(hours=4))
        self._create_spent_time(timezone.now() - timedelta(days=1, hours=5),
    ...
    def test_another_user_not_in_team(self):
        ...
        self.assertEqual(len(response.data), 2)
        self.assertFalse(
            any(item['user'] == another_user.id for item in response.data))

    def test_another_user_in_team(self):
        another_user = UserFactory.create()
        TeamMemberFactory.create(team=self.team, user=another_user,
                                 roles=TeamMember.roles.developer)
        ...
        developer_metrics = next(
            item for item in response.data if item['user'] == another_user.id)
