...
def test_issues_spents(user):
    ...
    assert summary.issues.opened_spent == timedelta(hours=5).total_seconds()
    assert summary.issues.closed_spent == timedelta(hours=3).total_seconds()
...
def test_merge_requests_spents(user):
    ...
    assert summary.merge_requests.opened_spent == timedelta(hours=5).total_seconds()
    assert summary.merge_requests.closed_spent == timedelta(hours=3).total_seconds()
    assert summary.merge_requests.merged_spent == timedelta(hours=6).total_seconds()
    assert summary.merge_requests.spent == timedelta(hours=14).total_seconds()

    assert summary.spent == timedelta(hours=14).total_seconds()
...
def test_complex_spents(user):
    ...
    MergeRequestSpentTimeFactory.create(
        ...
        base=MergeRequestFactory.create(user=user,
                                        state=MergeRequest.STATE.merged),
        time_spent=timedelta(hours=6).total_seconds()
    )
    ...
    assert summary.spent == timedelta(hours=22).total_seconds()
