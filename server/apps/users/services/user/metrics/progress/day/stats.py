...
class UserDayStatsProvider:
    ...
    def get_time_spents(
        ...
        queryset = (
            SpentTime.objects.annotate(day=TruncDay("date"))  # noqa: WPS221
            .filter(user=user, date__range=(start, end), day__isnull=False)
            .values("day")
            .annotate(period_spent=models.Sum("time_spent"))
            .order_by()
        )

        return {stats["day"]: stats for stats in queryset}
    ...
    def get_due_day_stats(self, user: User) -> Dict[date, Dict[str, int]]:
        ...
        queryset = (
            ...
            .filter(user=user, due_date_truncated__isnull=False)
            .values("due_date_truncated")
            .annotate(
        ...
        return {stats["due_date_truncated"]: stats for stats in queryset}
    ...
    def get_payrolls_stats(self, user: User) -> Dict[date, Dict[str, int]]:
        ...
        return {stats["date_truncated"]: stats for stats in queryset}
